  ...
  const connect = useCallback(async () => {
    return new Promise<void>((resolve, reject) => {
      try {
        ...
        ws.onopen = () => {
          console.log(`📡 WebSocket to edge function opened, waiting for Deepgram connection...`);
        };
        ...
        ws.onmessage = (event) => {
          try {
            ...
            // Wait for confirmation that edge function connected to Deepgram
            if (data.type === 'connection' && data.status === 'connected' && !isResolved) {
              console.log(`✅ Connected to Deepgram (${model}):`, data.message);
              ...
              setState(prev => ({ ...prev, isConnected: true, sessionId: data.request_id }));
              isResolved = true;
              resolve();
            }
